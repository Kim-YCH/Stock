...
### v11.6 Simplified Technical Analysis / Daily Portfolio Return

完成：

- StockLab 功能重新收斂為市場總覽、關注股票、候選清單、庫存管理與線圖分析
- 庫存新增前一交易日收盤價、今日漲跌、今日漲跌幅與持股今日損益
- 庫存總覽新增整體今日損益與整體今日漲跌幅
- 今日損益與未實現損益分開顯示，避免混淆
- 快速買賣表單改為按鈕開啟 modal，手機版使用 bottom sheet
- 持股列的買進與賣出按鈕可預先帶入股票代號、操作方向與目前價格
- 通知中心移除獨立頁籤，改為點擊右上角通知按鈕開啟小視窗
- 首頁初始化只讀通知未讀數，通知列表於開啟小視窗後才載入
- 暫時移除策略研究、策略健檢、策略回測、策略模型與模型比較前端功能
- 暫時停用策略與虛擬交易每日運算，但保留後端資料表與程式以供未來恢復
- 移除統計中心及其獨立頁面與初始化 API
- 移除候選排行、排名、策略模型標籤與模型信心
- 候選清單改為只使用 MA、RSI、MACD、KD、ADX、ATR、量比、布林與 SuperTrend 等技術指標判斷
- 買入候選只從 Watchlist enabled 股票產生
- 賣出候選只從 Portfolio quantity > 0 的持股產生
- 首頁移除策略警告、策略健康與模型候選卡片
- 線圖分析保留技術指標與目前判斷原因，不顯示策略命中資料
- 移除停用功能的初始化請求與每日快取重建，改善首頁與頁面載入速度
- 舊 strategy、stats、backtest、paper、leaderboard 路由會安全導向現有頁面
